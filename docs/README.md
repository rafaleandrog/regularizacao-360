...
**"Nenhum X" é afirmação, e há uma decisão só para todas.** Seis listas deste app terminam numa frase sobre o mundo — *"Nenhuma ação neste imóvel"*, *"Nenhuma transação neste imóvel"*, *"Nenhum morador"*, *"Nenhum lote cadastrado nesta instância"*, *"Nenhum lote neste parcelamento"*, *"Nenhuma ação com esta pessoa"*. Todas eram ditas também quando a leitura **falhava**, porque no `catch` a lista fica vazia e a flag de carregando volta a `false`. `estadoDaLista` (`comum/estado-lista.ts`) é a decisão única: a frase de vazio só sai com a leitura concluída, e nos outros dois estados entra uma mensagem própria mais a submensagem que nomeia o que a tela não sabe. Ter uma cópia por tela era o modo de elas divergirem — e cinco das seis já tinham divergido da sexta.

**Número não lido não é zero.** `numeroLido` (mesmo módulo) devolve `null`, nunca `0`, enquanto a leitura não concluiu. O caso que o motivou: *"0 pessoa(s) física(s) no Núcleo"* — afirmação sobre a base inteira, feita a partir de um total que a falha deixou em zero.
