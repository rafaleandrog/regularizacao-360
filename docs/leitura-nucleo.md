...
**A troca de rota zera o alvo antes de carregar o novo.** `this.rota` muda e só depois a carga corre; sem o reset síncrono no topo de `_carregar()`, a janela entre o clique e a resposta renderiza o cabeçalho, os KPIs, o preço e as propostas do **objeto anterior**, sob o nome do novo. É a única classe de defeito desta família que não exibe ausência — exibe **o dado de outra coisa**, com a mesma confiança. São zerados: `detalhe`, `propostas`, `vigente`, `dadosDoImovel`, `paiDoImovel`, `unidadesDoLote`, `avisoHerancaUnidade`, `loteDaUnidade`, `lotes`, `totalParcelamentosInstancia` (recalculado a cada vista, não do objeto), e os quatro estados de leitura: `leituraPropostas`, `leituraDadosDoImovel`, `leituraContextoDoImovel` e `leituraLotesDoParcelamento`. A lista é fechada porque cada item que fica de fora é um dado do objeto anterior sobrevivendo sob o nome do novo. Também são zerados `erro`, `avisoFlag`, `cargaFalhou` e `carregando`, mas ficam fora de propósito: são o estado da carga da view, não dado de um objeto — zerá-los aqui corresponde a começar uma carga nova, nunca a objeto anterior. `lotes` entrou depois dos demais, e por um motivo que a guarda nova tornou mais grave, não menos: sem o reset, navegar de um parcelamento para outro deixava a tabela do anterior na tela sob o nome do novo — e `estadoDaLista` não pega esse caso, porque lista **não-vazia** nunca dispara `podeAfirmarVazio: false`. A tabela continuaria com cara de resposta certa, só que do parcelamento errado.
...
**A lista da tela de Lotes tem um caso pior que o vazio: o cheio.** Quando a busca falha, os resultados da busca **anterior** continuam na tabela, agora sob o termo novo, com a paginação do total velho por baixo. Apagá-los perderia o que já foi lido; o que não pode é deixá-los se passarem pela resposta que não veio. A tela agora diz, num banner, que a tabela é o resultado anterior — e o rodapé de paginação diz a mesma coisa de outro jeito: `sufixoNumerosAntigos` (`comum/estado-lista.ts`) acrescenta `" (números da leitura anterior)"` a "Página X de Y · Z lotes" sempre que `leituraLotesGlobais` for `falhou`. É o mesmo sufixo, mesmo módulo, que a tela de Moradores usa na sua própria paginação (ver `docs/moradores.md`) — antes deste módulo, cada tela tinha a sua cópia, lendo o próprio estado sem checagem cruzada entre elas.

**A tabela de lotes do Parcelamento não podia usar `this.carregando` para essa mesma marca.** Essa flag é global à view de detalhe do Parcelamento: `_salvarRegularizacao`, `_acaoPreco`, `_acaoDeAcao` e o salvar de propostas todos a ligam durante o próprio request, na mesma tela, muito depois de os lotes já terem sido lidos. Derivar `estadoDaLista` de `this.carregando` fazia a tabela voltar a "Carregando…" cada vez que um desses modais salvava, com os lotes parados ali por baixo o tempo todo. `leituraLotesDoParcelamento` é o estado **próprio** dessa tabela, ligado só pelo request que busca `GET /lotes?parcelamento_id=` — a regra geral por trás é que estado de leitura é por **lista**, não por **view**: uma flag de tela inteira mistura o request certo com todos os outros que a mesma tela dispara depois dele.

**Resposta velha sobre rota nova é uma classe à parte — mesma família, defeito diferente.** O reset síncrono do topo de `_carregar()` cobre a janela entre o clique e a PRIMEIRA resposta. Ele não cobre a SEGUNDA navegação: se o usuário troca de rota de novo antes de a resposta anterior chegar, essa resposta atrasada não sabe que ficou velha — ela escreve por cima do estado da rota nova quando finalmente resolve, e o `catch` externo marca `cargaFalhou` da rota nova por uma falha que era da anterior. Diferente do caso do parágrafo anterior (nenhum reset, dado do objeto errado ficando exposto), aqui o reset aconteceu — só que depois dele, uma segunda carga já está em voo, e a primeira ainda não sabe disso.

A defesa é um **token de geração** (`comum/geracao-carga.ts`, `deveAplicarResposta`), a mesma forma já usada em dois lugares desta tela antes de existir um módulo próprio: o índice de Moradores (fila com uma varredura em voo por vez, PR #93) e a carga de regularização (uma requisição em voo por vez, PR #94) — ambos ficam como estão, por serem cache **entre views**, não estado da rota atual. `_carregar()` incrementa `this.geracaoCarga` no topo e guarda o valor capturado (`g`); depois de cada `await`, antes de escrever estado, confere `deveAplicarResposta({ geracaoDoPedido: g, geracaoAtual: this.geracaoCarga })` — se uma navegação mais nova já rodou, a resposta atual é descartada em silêncio, sem tocar `detalhe`, `lotes`, `cargaFalhou` nem nenhum outro campo. As cargas em segundo plano disparadas com `void` dentro de `_carregar()` (`_carregarPropostas`, `_carregarMatriculas`, `_carregarPessoasDaPagina`, `_carregarBasesDoVgv`, e as demais que escrevem estado do imóvel, do parcelamento ou da pessoa abertos) recebem o mesmo `g` por parâmetro — com valor padrão `this.geracaoCarga` para quem as chama fora de uma navegação (salvar proposta, aprovar, "tentar de novo", paginação), onde a geração atual é sempre válida por definição.

`this.carregando`, no `finally` de `_carregar()`, segue a mesma regra: só a própria geração o desliga. Uma resposta velha que chega depois de uma navegação nova não pode apagar o spinner de uma carga que ainda está em voo — e como a geração mais recente sempre chega ao seu próprio `finally` eventualmente (o encadeamento de chamadas é sempre finito), a flag nunca fica presa em `true`.
