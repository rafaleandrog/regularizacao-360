...
**A leitura tem três estados, não dois.** `regularizacaoLida` era booleano: falha e "ainda correndo" eram o mesmo `false`, sem retry. Agora é `leituraRegularizacao: EstadoContagem`, com `TEXTO_REGULARIZACAO_NAO_LIDA` para os dois estados sem dado. E a tela `/parcelamentos` passou a disparar a carga — antes só a home e o detalhe disparavam, e quem abria a lista direto via "fase não lida" para sempre.
